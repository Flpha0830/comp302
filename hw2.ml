...
let compress_tests : (nucleobase list * (int * nucleobase) list) list = [
  ([], []);
  ([A], [(1, A)]);
  ([A;A], [(2, A)]);
...
let decompress_tests : ((int * nucleobase) list * nucleobase list) list = [
  ([], []);
  ([(1, A)], [A]);
  ([(2, A)], [A;A]);
...
let to_instr_tests : (exp * instruction list) list = [
...
let rec to_instr: exp -> instruction list =
  function
...
let instr_tests: ((instruction * float list) * float list option) list = [
  ((Plus, [1.; 1.]), (Some [2.]) );
  ((Minus, [1.; 3.]), (Some [2.]));
  ((Cos, [0.]), (Some [1.]));
  ((Float 1., [1.]), (Some [1.; 1.]));
...
let prog_tests : (instruction list * float option) list = [
...
let prog (instrs : instruction list) : float option =
  let rec aux (i_list : instruction list) (s : stack) : stack option =
    match i_list with
    | i::instrs ->
      begin
